...
TimeSeriesTree::TimeSeriesTree( string  filename ) :	mFilename(filename),
    mConstraint(0),
    mDim(0),
 ...
 * the database
 */
TimeSeriesTree::TimeSeriesTree( string  filename, const uint constraint, const uint dim, const int parent_capacity, const int leave_capacity) :
    mFilename(filename),
...
vector<double> & TimeSeriesTree::readTimeSeries(uint myid) {
    assert(myid<mNextID);
...
bool TimeSeriesTree::good() {
    return mfile->good();
}

void TimeSeriesTree::add(const vector<double> &  newdata) {
 ...
 */
double TimeSeriesTree::getNearestNeighborCost(const vector<double> &  query, uint type, uint mode) {
    ++nbrqueries;
    ...
    if(type==NAIVE)
        ...
        cerr <<"I don't know this type"<<endl;
        throw "stop this!";
    }
...
uint TimeSeriesTree::getSequentialSearchCandidates()  const  {
    return candidates;
}

uint TimeSeriesTree::getNumberOfDTW() const {
    return nbrdtw;
}

uint TimeSeriesTree::getNumberOfCandidates() const {
    return mNextID*nbrqueries;
}

void TimeSeriesTree::resetStatistics() {
    nbrdtw=0;
    candidates = 0;
    nbrqueries=0;
}
